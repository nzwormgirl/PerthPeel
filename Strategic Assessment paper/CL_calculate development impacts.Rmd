...
calculate.protected <- function(input.scenario){
  ...
  protected <- cleared_area$protected.ha[cleared_area$scenario==input.scenario]
    protected.vegetation <- cleared_area$protected.[cleared_area$scenario==paste0(input.scenario,"_clipped")]
  ...
  for (h in 1:length(PA.levels)){
    ...
    output$min.dist[h] <- round(a[, 3]*100,2)
      #output$min.dist.cum[h] <- output$min.dist[h]
...
Original protected area analysis


```{r whats already protected}
  protected.curve.files <- Z.curves[grepl("protected",Z.curves)]

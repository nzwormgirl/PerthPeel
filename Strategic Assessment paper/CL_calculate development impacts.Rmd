...
# load horrenda table
  ...
  unprotected.species <- protected.species[protected.species$Scientific.Name %in% unprotected.sp, c("Scientific.Name","AcceptedCommonName","Taxa","mnes","WA_Status","EPBC_Status","Endemism","datatype","EPBC_ListedMigratory",	"EPBC_ListedMarine")]
    ...
    unprotected.species$EPBC.Status <- gsub("^\\,\\s", "",unprotected.species$EPBC.Status)
...
# match scenario with mask layer
  ...
  loss.table$order <- c(8,13,9,14,10,15,11,16,12,17,1,2,3,4,5,6,7)
...
special.sp.table$fraction <- (loss.table[match(loss.table$scenario,special.sp.table$scenario),"area.cleared"])/100
...
write.csv(special.sp.table,paste0(output_path,"tables/Figure4_special.sp.table_",analysis.date,".csv"),row.names=F)

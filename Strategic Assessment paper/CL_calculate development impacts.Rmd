...
```{r useful functions}
  ...
  clean.names <- function (x) trim(gsub("_"," ",gsub("|PP|\\.tif|SSI\\.|\\/|PP\\.", "", x)))
...
tryCatch({
  cleared_area <- read.csv(paste0(output_path,"tables/cleared_area_",analysis.date,".csv"))
      ...
      cleared_area <- data.frame(scenario=scenario.names,cleared=NA,cleared.ha=NA,protected=NA,protected.ha=NA,total=NA)
        ...
        write.csv(cleared_area,paste0(output_path,"tables/cleared_area_",analysis.date,".csv"), row.names = F)
      })
...
# load horrenda table
  ...
  unprotected.species <- protected.species[protected.species$Scientific.Name %in% unprotected.sp, c("Scientific.Name","AcceptedCommonName","Taxa","mnes","WA_Status","EPBC_Status","Endemism","datatype","EPBC_ListedMigratory",	"EPBC_ListedMarine")]
    ...
    for(i in seq(status)){
      ...
      if(i==2) {
        marine <- unprotected.species[!is.na(unprotected.species$mnes),"EPBC_ListedMarine"]
        migratory <- unprotected.species[!is.na(unprotected.species$mnes),"EPBC_ListedMigratory"]
    ...
    unprotected.species$EPBC.Status <- gsub("^\\, ", "",unprotected.species$EPBC.Status)
  ...
  write.csv(unprotected.species, paste0(output_path, 'tables/CL_unprotected_species_',analysis.date,'.csv'), row.names=F)
...
# match scenario with mask layer
  ...
  loss.table$scenario <- c("1.1 EIA1","1.2 EIA2", "1.3a EIA3 Option1", "1.3b EIA3 Option2", "1.4 Development Footprint", "1. Urban","2. Industrial","3. Rural Residential", "4. Infrastructure", "5. Forestry", "6. Mining", "7. Cumulative")
  ...
  try({
    ...
    # identify curve data associated with cleared area
      ...
      loss.table$`median.loss(%)`[h] <- round(100-(median(as.numeric(data.frame(a[,8:ncol(a)])
))*100),2)
      loss.table$`25%.sp.loss`[h] <- round(100-(quantile(as.numeric(data.frame(a[,8:ncol(a)])
),0.25))*100,2)
  ...
  loss.table$protected.areas <- conflict.table$prop.protected
  loss.table$priority.areas <- conflict.table$prop.top
...
```{r Tables for ms}
 ...
 Table1 <- data.frame("Scenario"=c("1. Urban","2. Industrial","3. Rural Residential", "4. Infrastructure", "5. Forestry", "6. Mining", "7. Cumulative"),	"Area.developed.(ha)"=NA,"Habitat.cleared.(ha)"=NA,	"Development.conflicts.Protected.Areas"=NA, "Development.conflicts.Priority.Areas"=NA, "Loss.feature.distributions.mean"=NA,"Loss.feature.distributions.min"=NA,"Locally.extinct.features"=NA,"Features.losing.90%"=NA,"Features.losing.75%"=NA,"Features.losing.50%"=NA,mask.layer=)
 
```



```{r estimate offsets, eval=F}

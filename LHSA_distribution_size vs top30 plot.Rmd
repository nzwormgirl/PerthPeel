...
input_path <- paste0(computer,'/GIS_data/Hunter/zonation/lower hunter/')
output_path <- paste0(computer,'/GIS_data/Hunter/zonation/lower hunter/Output/')
...
LH.lakes.mask <- raster(paste0(computer,"/GIS_Data/Hunter/mask files/LH.lakes.mask.tif"))
...
# open tec list
  tec.data <- read.csv(paste0(computer,"/GIS_data/Hunter/species point data/GreaterHunter_DRAFT_100812 (2).csv"))
    tec.data <- tec.data[!duplicated(tec.data$TEC_Amy),c("TEC_Amy","points","LH.points","TECName","weight")]
    tec.data$TEC_Amy <- tolower(tec.data$TEC_Amy)

unweighted.priority <- raster(paste0(output_path,"output_LH_040215.rank.compressed.tif"),crs=GDA94.56)
weighted.priority <- raster(paste0(output_path,"output_LH_weighted_040215.rank.compressed.tif"),crs=GDA94.56)
...
strip.names <- function(x){
  trim(gsub("_"," ",gsub("extant\\/|\\.LH|\\.tif|SSI|SDM|\\_LH|DotE\\/","",x)))
}
...
for(j in seq(PA.steps)){
  ...
  for(i in 1:3){
    cleared_area$scenario[row] <- gsub("//654cw-20990/Amy/GIS_data/Hunter/zonation/lower hunter/masks/","",paste0(PA.steps[j]," Level ",i))
      r.max <- cellStats(r,max)
...
weighted.curves <- read.table(paste0(output_path,"output_LH_weighted_040215.curves.txt"), skip=1, header=F, sep='')
...
unweighted.curves <- read.table(paste0(output_path,"output_LH_040215.curves.txt"), skip=1, header=F, sep='')
...
protected.curves <- read.table(paste0(output_path,"output_LH_weighted_PA_050215.curves.txt"), skip=1, header=F, sep='')
...
# at risk species
  ...
  write.csv(protected.fractions, paste0(output_path,"LHSA_protected.table.csv"),row.names=F)
...
n.cells <- length(LH.clipping.mask[!is.na(LH.clipping.mask)])

weighted.curves <- read.table(paste0(output_path,"output_LH_weighted_040215.curves.txt"), skip=1, header=F, sep='')
      colnames(weighted.curves) <- c('Prop_landscape_lost', 'cost_needed_for_top_fraction', 'min_prop_rem', 'ave_prop_rem', 'W_prop_rem', 'ext-1', 'ext-2', names)

distribution.size <- read.csv(paste0(output_path,"output_LH_weighted_040215.features_info.csv"))
...
distribution.size$scalar <- 1
  ...
  distribution.size$scalar[distribution.size$MapFileName == "DotE/ghff_foraging_LH.tif"] <- 4
  distribution.size$scalar[distribution.size$MapFileName == "DotE/koala_rawhabval6_LH.tif"] <- 2200.782
  distribution.size$scalar[grepl("DotE/Regent_Honeyeater_LH.tif|DotE/Swift_Parrot_LH.tif",distribution.size$MapFileName)] <- 1000
...
png(paste0(output_path,"Protected_Areas.png"),height=9,width=15,units="cm",res=300, bg="transparent",pointsize=10)
  ...
  plot(LH.lakes.mask,col=map.background,legend=F,box=F,axes=F)
    plot(protected.areas,col=pa.colours$colour[1:3],add=T,legend=F)
    plot(LH.shp, add=T, lwd=0.75)
...
png(paste0(output_path,"unweighted & weighted priorities.png"),height=17.5,width=15,units="cm",res=300, bg="transparent",pointsize = 10)
  ...
  plot(LH.lakes.mask,col=map.background,legend=F,box=F,axes=F,zlim=c(1,6))
...
distribution.size$weight <- as.vector(read.table(paste0(input_path,'LH.extant.zonation.weighted.spp'), header=F, sep='\t')[,1])
...
lga.areas <- data.frame(lga=LH.shp$LGAName,total.area=NA,veg.area=NA,top.priorities=NA)
...
png(paste0(output_path,"weighted priorities by LGA.png"),height=17.5,width=15,units="cm",res=300, bg="transparent",pointsize = 10)
  ...
  for(i in seq(LH.shp$LGAName)){
    lga <- LH.shp$LGAName[i]
    lga.mask <- mask(crop(LH.mask,LH.shp[grepl(lga,LH.shp$LGAName),]),LH.shp[grepl(lga,LH.shp$LGAName),])
      lga.areas$total.area[i] <- length(lga.mask[!is.na(lga.mask)])
    lga.mask <- lga.mask*LH.urban.clipping.mask
    
    lga.veg.mask <- mask(crop(LH.clipping.mask,LH.shp[grepl(lga,LH.shp$LGAName),]),LH.shp[grepl(lga,LH.shp$LGAName),])
      lga.areas$veg.area[i] <- length(lga.veg.mask[!is.na(lga.veg.mask)])
    
    lga.priority <- mask(crop(weighted.priority,LH.shp[grepl(lga,LH.shp$LGAName),]),LH.shp[grepl(lga,LH.shp$LGAName),])
      ...
      plot(lga.mask,col=map.background,legend=F,box=F,axes=F)
        ...
        plot(LH.shp[grepl(lga,LH.shp$LGAName),], add=T, lwd=0.75)
...
lga.areas$relative.area <- lga.areas$total.area / length(LH.mask[!is.na(LH.mask)])
lga.areas$relative.veg <- lga.areas$veg.area / length(LH.clipping.mask[!is.na(LH.clipping.mask)])
...
lga.areas$high.priority.veg.lhsa <- lga.areas$top.priorities / length(LH.clipping.mask[!is.na(LH.clipping.mask)])
...
par(mar=c(0,0,0,0), oma=c(0,0.5,0,0.5))
    ...
    image(LH.lakes.mask,col=map.background,axes=F,ann=F)
...
#       text(x=interest.points$x[4],y=interest.points$y[4],"4",font = 2,cex=1.5)
  ...
  par(mar=c(1,1,2,1)) 
  image(LH.lakes.mask,col=map.background,ann=F,axes=F,xlim=c(interest.points$x[1]-6000,interest.points$x[1]+6000),ylim=c(interest.points$y[1]-6000,interest.points$y[1]+6000),asp=1)
    image(weighted.priority,col=pri.col,breaks=pri.breaks,ann=F,add=T,asp=1)
    plot(LH.shp, add=T, lwd=0.5)
    box()
    mtext("1",side=3,line=-1.5,adj=0.05)

  image(LH.lakes.mask,col=map.background,ann=F,axes=F,xlim=c(interest.points$x[2]-6000,interest.points$x[2]+6000),ylim=c(interest.points$y[2]-6000,interest.points$y[2]+6000),asp=1)
    image(weighted.priority,col=pri.col,breaks=pri.breaks,ann=F,add=T,asp=1)
    plot(LH.shp, add=T, lwd=0.5)
    box()
    mtext("2",side=3,line=-1.5,adj=0.05)

  image(LH.lakes.mask,col=map.background,ann=F,axes=F,xlim=c(interest.points$x[6]-6000,interest.points$x[6]+6000),ylim=c(interest.points$y[6]-6000,interest.points$y[6]+6000),asp=1)
    image(weighted.priority,col=pri.col,breaks=pri.breaks,ann=F,add=T,asp=1)
    plot(LH.shp, add=T, lwd=0.5)
    box()
    mtext("3",side=3,line=-1.5,adj=0.05)

  image(LH.lakes.mask,col=map.background,ann=F,axes=F,xlim=c(interest.points$x[5]-6000,interest.points$x[5]+6000),ylim=c(interest.points$y[5]-6000,interest.points$y[5]+6000),asp=1)
    image(weighted.priority,col=pri.col,breaks=pri.breaks,ann=F,add=T,asp=1)
    plot(LH.shp, add=T, lwd=0.5)
    box()
...
protected.priorities <- mask(protected.priorities+ protected.areas.plot*4,LH.lakes.mask)
...
important.species <- list(distribution_size_threshold=threshold,box1=cbind(protected.species[protected.species$Scientific.Name %in% strip.names(distribution.size$MapFileName[distribution.size$box1>threshold]),c("Scientific.Name","Common.Name","mnes","NSW.status","Comm.status")],distribution.size=distribution.size$relative.distribution[distribution.size$box1>threshold],prop.dist=distribution.size$box1[distribution.size$box1>threshold]),
    ...
    box4=cbind(protected.species[protected.species$Scientific.Name %in% strip.names(distribution.size$MapFileName[distribution.size$box4>threshold & distribution.size$MapFileName!="extant/Ninox_connivens_SSI.LH.tif"]),c("Scientific.Name","Common.Name","mnes","NSW.status","Comm.status")],distribution.size=distribution.size$relative.distribution[distribution.size$box4>threshold & distribution.size$MapFileName!="extant/Ninox_connivens_SSI.LH.tif"],prop.dist=distribution.size$box4[distribution.size$box4>threshold & distribution.size$MapFileName!="extant/Ninox_connivens_SSI.LH.tif"]))
...
zonation.features <- data.frame(Taxa=NA,Scientific.Name=strip.names(names),Common.Name=NA,MNES=NA,Commonwealth.Status=NA,NSW.Status=NA,No.Records=NA,Data.type=NA,Weight=NA)

zonation.features$Scientific.Name <- as.character(zonation.features$Scientific.Name)

zonation.features$Scientific.Name[grepl("Regent Honeyeater",zonation.features$Scientific.Name)] <- "Anthochaera phrygia"
zonation.features$Scientific.Name[grepl("Swift Parrot",zonation.features$Scientific.Name)] <- "Lathamus discolor"

for(i in seq(zonation.features$Scientific.Name)){
  try({
    species <- zonation.features$Scientific.Name[i]
  zonation.features$Taxa[i] <- as.character(protected.species$Taxa[grepl(species,protected.species$Scientific.Name)])
  zonation.features$Common.Name[i] <- as.character(protected.species$Common.Name[grepl(species,protected.species$Scientific.Name)])
  zonation.features$MNES[i] <- protected.species$mnes[grepl(species,protected.species$Scientific.Name)]
  zonation.features$Commonwealth.Status[i] <- as.character(protected.species$Comm.status[grepl(species,protected.species$Scientific.Name)])
  zonation.features$NSW.Status[i] <- as.character(protected.species$NSW.status[grepl(species,protected.species$Scientific.Name)])
  zonation.features$No.Records[i] <- paste0(protected.species$lower.hunter[grepl(species,protected.species$Scientific.Name)]," (",protected.species$greater.hunter[grepl(species,protected.species$Scientific.Name)],")")
  zonation.features$Weight[i] <- protected.species$weight[grepl(species,protected.species$Scientific.Name)]
  },silent=T)
}

zonation.features$Data.type[grepl("SDM|Regent Honeyeater|Swift Parrot",names)] <- "SDM"
zonation.features$Data.type[grepl("SSI",names)] <- "SSI"
zonation.features$Data.type[grepl("Nominated",names)] <- "Polygon"
zonation.features$Data.type[is.na(zonation.features$Data.type)] <- "Other"
zonation.features$No.Records[grepl("Regent Honeyeater|Swift Parrot",zonation.features$Common.Name)] <- NA


missing.records <- zonation.features$Scientific.Name[is.na(zonation.features$No.Records)]

tec.data$NSW.Status <- NA
tec.data$Commonwealth.Status <- NA
tec.data$MNES <- NA

for(i in seq(tec.data$TECName)){
  tec.data$Threat.Status[i] <- strsplit(as.character(tec.data$TECName[i]),":")[[1]][1]
  if(grepl("EPBC",tec.data$Threat.Status[i])==TRUE) {
    tec.data$Commonwealth.Status[i] <- strsplit(as.character(tec.data$Threat.Status[i]),",")[[1]][2]
    tec.data$MNES[i] <- "TRUE"
    } else {
    tec.data$NSW.Status[i] <- strsplit(as.character(tec.data$Threat.Status[i]),",")[[1]][2] 
    }
}


for(i in seq(zonation.features$Scientific.Name)){
  if(is.na(zonation.features$NSW.Status[i])==TRUE){
    try({
      species <- gsub(" ","_",zonation.features$Scientific.Name[i])
      zonation.features$NSW.Status[i] <- as.character(tec.data$NSW.Status[grepl(species,tec.data$TEC_Amy)])
      zonation.features$Commonwealth.Status[i] <- as.character(tec.data$Commonwealth.Status[grepl(species,tec.data$TEC_Amy)])
      zonation.features$MNES[i] <- as.character(tec.data$MNES[grepl(species,tec.data$TEC_Amy)])
    zonation.features$No.Records[i] <- paste0(tec.data$LH.points[grepl(species,tec.data$TEC_Amy)]," (",tec.data$points[grepl(species,tec.data$TEC_Amy)],")")
    zonation.features$Weight[i] <- tec.data$weight[grepl(species,tec.data$TEC_Amy)]
      })  
  }

}

write.csv(zonation.features,paste(output_path,"Summary of zonation input features.csv"),row.names=F)

table(zonation.features$Data.type)
```

```{r species richness & spatial uncertainty}

# gh.sdms <- gsub("extant\\/","",gsub(".LH","_GH",sdm.species))
# 
...
#   writeRaster(species.richness,paste0(output_path,"LH.species.richness.tif"))

species.richness <- raster(paste0(output_path,"LH.species.richness.tif"))
mean.uncertainty <- raster(paste0("//654cw-20990/Amy/GIS_data/Hunter/Maxent_files/ghm.pm/all_variables/output/LH.mean.uncertainty_2015-02-08.tif"))
...
LH.mean.uncertainty <- mask(mean.uncertainty,LH.shp)

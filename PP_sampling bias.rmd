...
setwd("~/GIS_data/Perth-Peel/environmental data/100m/ibra")
...
pp.ibra <- raster("~/GIS_data/Perth-Peel/environmental data/pp_ibra_mask")
  pp.ibra[!is.na(pp.ibra)] <- 0

plot(pp.ibra,col="grey",legend="F")
points(maxent.data, pch=".")

maxent.raster <- rasterize(maxent.data,pp.ibra,field=1,update=T)
plot(maxent.raster)
```
...
r_filt <- mask(focal(maxent.raster, w=cf, fun=percent.cover),pp.ibra)

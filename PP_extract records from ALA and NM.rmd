...
protected.species <- read.csv("WA listed protected species.csv")
  colnames(protected.species)[5] <- "Scientific.Name"
...
taxa <- c("mammals", "amphibians", "plants", "reptiles","birds","invertebrates")
...
for (k in seq(taxa)){
  ...
  P.input <- read.csv(paste0("ALA downloads/Perth ",taxa[k],".csv"))
  NJF.input <- read.csv(paste0("ALA downloads/NJF ",taxa[k],".csv"))
  DP.input <- read.csv(paste0("ALA downloads/DP ",taxa[k],".csv"))

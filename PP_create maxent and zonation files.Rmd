...
# set the google map background for the species plots and create background map
  map <- get_map(location = c(115.85,-32.18), zoom = 8, maptype="terrain")
  ...
  print(base.map <- ggmap(map) + geom_polygon(aes(x = long, y = lat, group = group), data=PP.mask.data, colour = 'yellow', fill = 'black', alpha = .1, size = .3))  
...
# open species list
  protected.species <- read.csv("C:/Users/awhitehead/Documents/GIS_data/Perth-Peel/species data/WA listed protected species.csv")

threatened.species <- protected.species

#   threatened.species <- rbind(protected.species[grep("E", protected.species$NM.status),], protected.species[grep("V", protected.species$NM.status),], protected.species[grep("TRUE", protected.species$mnes),])
...
species.list <- threatened.species$Scientific.Name[rowSums(threatened.species[,c("ALA.records","NM.records")],na.rm=TRUE) >0 & threatened.species$ignore!="Y" ]
...
for (i in seq(species.list)){
  ...
  if(nrow(ALA.NM.data) > 0) {
    ...
    if(in.PP > min.records) {
      sp.map <- base.map + geom_point(aes(x = longitude, y = latitude,color=database, shape=species), data = PP.sp, alpha=0.5)
      print(sp.map)
    
    # output data for maxent analyses
      atlas.maxent.data <- rbind(atlas.maxent.data,PP.sp)
      cat("\n", "Adding", in.PP, "records for", as.character(species.list[i]), "to maxent data")
        ...
        if(0 < in.PP & in.PP < min.records) {
          ...
          ssi.zonation.data <- rbind(ssi.zonation.data, PP.sp)
          cat("\n", "Adding", in.PP, "records for", as.character(species.list[i]), "as species of special interest")
        }
        }
    
    # remove all species-specific datafiles, so they don't mess up the next round of the loop
    suppressWarnings(rm(ALA.data, NM.data, ALA.NM.data, hccrem.sp,in.hunter,sp.map))
    gc()
  }
}
...
# convert maxent data to GDA94 Transverse Mercator Zone 56 to match the rest of the data
  ...
  PP.raster <- raster("C:/Users/awhitehead/Documents/GIS_Data/Perth-Peel/environmental data/region.mask.250.asc")
  ...
  export_ssi.zonation.data <- maxent.data # catch the species that get dropped due to duplicate removal
    ...
    for(i in seq(unique(gda.tm_maxent.data$species))){
      ...
      cell <- cellFromXY(PP.raster,input[,2:3])
...
if(save.output ==TRUE) {
  ...
  for (i in seq(ssi.export.species)) {
    ...
    write.table(export[,!names(export) %in% c("species")], paste0("C:/Users/awhitehead/Documents/GIS_data/Perth-Peel/zonation/ssi_files/",ssi.export.species[i],".txt"),row.names=FALSE, sep="\t", col.names=FALSE)
  }
   
  write.csv(protected.species, "C:/Users/awhitehead/Documents/GIS_data/Perth-Peel/species data/WA listed protected species.csv", row.names=FALSE)

cat("\n", nrow(export_ssi.zonation.data), "records exported to", length(ssi.export.species), "individual csv files for ssi from pool of",length(species.data.list), "species with available data")
  }
...
if(save.output ==TRUE) {
  write.table(zonation.spp,paste0("C:/Users/awhitehead/Documents/GIS_data/Perth-Peel/Maxent_files/species_data/zonation.spp"),sep="\t",row.names=FALSE, col.names=FALSE)
    write.table(mnes.zonation.spp,paste0("C:/Users/awhitehead/Documents/GIS_data/Perth-Peel/Maxent_files/species_data/mnes.zonation.spp"),sep="\t",row.names=FALSE, col.names=FALSE)
  write.table(ssi.zonation.spp,paste0("C:/Users/awhitehead/Documents/GIS_data/Perth-Peel/Maxent_files/species_data/ssi.zonation.spp"),sep="\t",row.names=FALSE, col.names=FALSE)
  write.table(mnes.ssi.zonation.spp,paste0("C:/Users/awhitehead/Documents/GIS_data/Perth-Peel/Maxent_files/species_data/mnes.ssi.zonation.spp"),sep="\t",row.names=FALSE, col.names=FALSE)
}
...
*This file was last updated on 24 May 2013 and last run on `r format(Sys.time(), "%d %B %Y")`.*
